...
- **Client review addendum (2026-09-04, feedback batches F1–F4):** সাতটা feedback যাচাই করা হলো; দুইটা ইতিমধ্যেই ঠিক ছিল (map default Dhaka-তেই ছিল — `DEFAULT_CENTER` 23.8103/90.4125; আর unit price বরাবরই user-ই দিত, per-sqft rate বা fixed — সমস্যা ছিল অন্য জায়গায়)। একটাই schema addendum, বাকিগুলো display ও guard:
  ...
  - **Tier 3.1 — Material item catalogue (2026-09-05), Dexie v13:** নতুন `material_items` table, আর যেসব table-এ item-এর নাম free text ছিল সেগুলোতে nullable `item_id`। §6.6 শুরু থেকেই free text-কে সাময়িক বলেছিল — কারণ "Cement (Fresh)" আর "cement fresh" দুইটা আলাদা item হয়ে আলাদা quantity ও আলাদা weighted-average cost ধরে রাখত, আর stock valuation নিঃশব্দে অর্থহীন হয়ে যেত
  - **সিদ্ধান্ত ১ — stock-এর পরিচয় বানান থেকে item-এ সরল, আর unit item-এর উপরে উঠল:** §7.7-এ stock row-এর key ছিল `(project_id, item_name, unit)`, তাই এক material-এর যত বানান তত row। এখন key `(project_id, item_id)`। Unit হলো material-এর ধর্ম — সিমেন্ট bag-এ থাকে — আর key-তে unit থাকাটাই ছিল এক material-কে ভাগ করার তৃতীয় রাস্তা; এটাই একমাত্র উপাদান যেটা উন্নত করার বদলে **সরিয়ে** দেওয়া যায়। অন্য unit-এ কেনা মানে conversion, যেটা scope-এর বাইরে; এক item-এ দুই unit রাখলে সমস্যাটাই ফিরে আসত। Stock থাকা অবস্থায় unit বদলানো আটকানো হয়, কারণ `average_unit_price` per unit — bag থেকে ton করলে store-এর দাম বিশগুণ হয়ে যেত
  - **সিদ্ধান্ত ২ — migration back-fill করে, "ধীরে ধীরে মেলানো" নয়:** §3.1-এ progressive matching-এর কথা ছিল। অর্ধেক migrate হওয়া table সবচেয়ে খারাপ — `findRow`-কে কিছু row-এর জন্য item আর কিছুর জন্য বানান ধরে চলতে হতো **চিরকাল**, আর ইতিমধ্যে যে দুই বানান মেলানোর জন্য এই feature, সেগুলো আলাদা stock ধরেই থাকত। Pass-টা আগে `stock` থেকে catalogue বানায় (যে row-গুলোতে সত্যিই quantity ও cost আছে), তারপর issue/transfer/PO line মেলায়। পুরোটাই additive — row যোগ করে আর একটা nullable column ভরে, কোনো quantity বা দাম ছোঁয় না। `findRow`-এ নামের fallback থেকেই যায়, তাই যা বাদ পড়ে তা-ও চলতে থাকে
  - **বর্তমান অবস্থা নতুন নাম দেখায়, দলিল পুরনোটাই রাখে:** item rename করলে stock page ও picker বদলায় (stock মানে "এখন store-এ কী আছে"), কিন্তু purchase order যে নামে order হয়েছিল সেটাই দেখায় (PO একটা দলিল)। এই কারণেই প্রতিটা line row-তে `item_id`-র পাশে `item_name` রাখা আছে
  - **Duplicate ঠেকানো হয়, merge করা হয় না:** নাম case-insensitive unique (`lookupRepository.addOption`-এর মতোই)। দুইটা catalogue item **merge** করা ইচ্ছা করে বানানো হয়নি — ওতে দুইটা weighted average জোড়া লাগাতে হয়, আর Module 6-এ আগেই সিদ্ধান্ত আছে যে running average reversible না (GRN delete করলেও ওটা ফেরে না)। Merge করতে গেলে এমন একটা দাম বানাতে হতো যেটায় কোনো material-ই কেনা হয়নি। OPEN-ITEMS-এ খোলা রাখা হলো
  - **Plan-এর তিনটা দাবি code-এর সাথে মেলেনি:** (১) §3.1 বলে ছয়টা table-এ item নাম লেখা হয় — `goods_receipt_items`-এ item নাম নাই, ওটা `po_item_id` দিয়ে পৌঁছায়, তাই পাঁচটা; (২) "item pickers on 5 forms" আসলে দুইটা — `StockIssueModal` ও `StockTransferModal` আগে থেকেই বিদ্যমান stock row থেকে বাছে; (৩) `project_budget_lines`-এর v13 ভাগাভাগি করার কথা ছিল, কিন্তু যে feature বানানোই হয়নি তার জন্য খালি table ঘোষণা করলে এমন index আটকে যেত যা কেউ design করেনি — তাই **3.2 এখন v14, আর 3.5 v15**

  - **Tier 3.2 — Project budget / BOQ (2026-09-05), Dexie v14:** নতুন `project_budget_lines` (`project_id`, `cost_category`, `budgeted_amount`, `notes`), index `[project_id+cost_category]` — একটা project প্রতিটা head-এ একবারই budget করে
  - **Plan-এ যে সমস্যাটার কথা নাই, সেটাই feature-টার আকার ঠিক করেছে — procurement খরচের কোনো `cost_category` নাই:** project-এর খরচ আসে দুই জায়গা থেকে — expense ledger (যার category আছে) আর `supplier_vouchers` (যার নাই, §8.3 roll-up-এ যোগ করে)। শুধু categorised expense ধরে budget করলে বেশিরভাগ project-এর **সবচেয়ে বড় লাইনটাই** (material) বাদ পড়ত, অথচ budget-টা সম্পূর্ণ দেখাত। তাই একটা reserved head রাখা হলো — `_procurement`
  - **শুরুর underscore-টা কাজের, সাজানোর জন্য না:** `lookupRepository` নতুন cost category-র slug বানানোর সময় শুরুর underscore ছেঁটে ফেলে, তাই কেউ "Procurement" নামে category বানালে সেটা `procurement` হয় — `_procurement`-এর সাথে কখনো ধাক্কা খায় না। Head-টা generator-এর নাগালের বাইরে, তাই আলাদা কোনো guard লাগেনি (guard যোগ করার চেয়ে এটাই ভালো)
  - **`stock_issues` ইচ্ছা করে গোনা হয় না:** site-এ পাঠানো material-এর দাম আগেই ওই voucher-এ দেওয়া হয়ে গেছে যেটা দিয়ে কেনা হয়েছিল; store থেকে tower-এ যাওয়ার সময় আবার গুনলে §8.3 যে double count-এর কথা বলে ঠিক সেটাই হতো
  - **Budget না করা head লুকানো হয় না, আলাদা করে দেখানো হয়:** যে head-এ খরচ আছে কিন্তু budget line নাই সেটা নিজের row পায় (badge সহ) আর একটা warning, আর Spent-এ গোনা হয় যাতে শতাংশটা সৎ থাকে। যে budget নিঃশব্দে uncategorised খরচ গিলে ফেলে সেটা সম্পূর্ণ দেখায় অথচ টাকা বেরিয়ে যেতে থাকে — এই একটা জিনিসই এই screen-এর লুকানো চলে না
  - **Budget কী নতুন প্রশ্নের উত্তর দেয়:** overview card আগের মতোই "estimated profit so far" বলে, যেটা মাঝপথের project-এ সবসময়ই লোকসান দেখাত। Budget tab বলে **profit at completion** — sales value বাদ *পরিকল্পিত* খরচ। দুইটা ইচ্ছা করেই আলাদা, আর label-এ সেটা লেখা
  - **Budget flat, পুরো BOQ না (খোলা রাখা হলো):** প্রতি project-এর প্রতি cost head-এ একটা অঙ্ক, কারণ actual দিকটাই কেবল category ধরে রাখা হয় — এর চেয়ে সূক্ষ্ম budget করলে এমন variance বেরোত যা কখনো হিসাব করাই যেত না। Quantity ও rate সহ আসল BOQ করতে হলে আগে actual দিকটাকে একই structure বইতে হবে

  - **Tier 3.5 — Bank account ও Cash position (2026-09-05), Dexie v15:** নতুন `bank_accounts`, আর টাকার নড়াচড়া ধরে রাখে এমন চারটা table-এ indexed `account_id` (`payments`, `expenses`, `supplier_vouchers`, `refunds`)। Index দেওয়া হয়েছে কারণ একটা account-এর statement চারটা table থেকে পড়তে হয় — index ছাড়া একটা statement আঁকতে গিয়ে সব payment scan করা demo-তে চলে, office-এ চলে না
  - **`stores()` একটা table-এর পুরো index set বদলে দেয়:** v15-এর প্রথম খসড়ায় `payments`-এর line-টা `payment_method` আর `received_by` index দুইটা নিঃশব্দে ফেলে দিচ্ছিল। এখন প্রতিটা v15 spec আগেরটার superset কিনা যাচাই করা হয়েছে — ভবিষ্যতের প্রতিটা version block-এ এই check-টা আবার করা উচিত
  - **§4.0 আগে সংজ্ঞা লিখতে বলেছিল, তাই সেটা হিসাবের পাশেই লেখা আছে:** চারটা table **পরস্পর বিচ্ছিন্ন** — একটা buyer receipt কখনো voucher-ও না — তাই receipt যোগ করে expense/voucher/refund বাদ দিলে প্রতিটা টাকা একবারই গোনা হয়। **গোনা হয় না:** `payment_installments` (কী পাওনা, কী নড়েছে তা না), `stock_issues`/`stock_transfers` (যে voucher দিয়ে কেনা হয়েছিল সেখানেই দাম দেওয়া হয়ে গেছে), budget line (পরিকল্পনা)। Refund থেকে `net_refund` যায়, `amount` না — deduction তো account থেকে বেরই হয়নি
  - **§4.0-এর আশঙ্কার double count সত্যি, কিন্তু ওটা data-entry duplicate:** এক payment-কে expense আর voucher দুই জায়গায় লিখলে দুইটা row হয়, আর সেটা একই অঙ্কের দুইটা আসল payment থেকে আলাদা করা যায় না — কোনো filter দিয়ে ঠিক করার জিনিস না
  - **যে double count-টা আসলেই ঘটতে পারে, §4.0 সেটার কথা বলেনি:** account-এর opening balance-এর **আগের** movement ওই balance-এর ভিতরেই আছে, দুইটাই গুনলে একই ইতিহাস দুইবার যোগ হতো। তাই opening date থেকে movement ছাঁকা হয়, আর test-এ দেখানো হয়েছে naive যোগফল আলাদা আসে
  - **`opening_balance` plan-এর schema-তে ছিল না, কিন্তু ওটা ছাড়া "position" মানে শুধু software বসানোর পর থেকের net movement** — যেটা কোনো bank statement-এর সাথে মেলানো যায় না
  - **যে টাকা কোনো account দিয়ে যায়নি সেটা আলাদা করে দেখানো হয়, balance-এ ঢোকে না** (3.2-এর unbudgeted spend-এর মতোই নিয়ম) — নাহলে যে অঙ্কটা দেখা যেত সেটা কোনো bank statement-এর সাথেই মিলত না, অথচ page-টার পুরো কাজই তো ওটা
  - **VAT/AIT rider যোগ করা হলো, memo field হিসেবে:** `amount` যা সত্যিই account থেকে বেরিয়েছে তা-ই থাকে, তাই এই দুইটা ভরা হোক বা না হোক cash position ঠিক থাকে। `amount`-কে gross বানিয়ে payment হিসাব করলে এমন একটা column-এর মানে বদলে যেত যেটা ইতিমধ্যে সব screen পড়ে
